...
function output = greedy_cos(maxtime,T,epsilon,n,dim,points,mu,c_var,mean,var,K,delta,partion,functype,option)
...
while (t < (T+1))&&(calctime < maxtime)
    ...
    for k=1:K
       ...
       d_k_x_t=d_k_x_t + lambda*inner_product(c,[1],p,p_d_k,epsilon,dim,functype);
       d_k_mu=d_k_mu+lambda*mu(ind_v);
       d_k_norm=d_k_norm + lambda^2*basevalue +2*lambda*inner_product(coeff_d_copy,[1],p_d_copy,p_d_k,epsilon,dim,functype) ; 
