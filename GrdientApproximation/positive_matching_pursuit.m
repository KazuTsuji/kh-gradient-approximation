...
function output = positive_matching_pursuit(maxtime,T,epsilon,n,dim,points,mu,c_var,mean,var,K,delta,partion,functype,option)
...
while ((length(c)+sum(same_count)) < (T+1))&&(calctime < maxtime)
    %%%
    p_d= zeros(K,dim);
    coeff_d=[];
    ind_d=[];
    ...
    for k=1:K
       ...
       if (value1 > value2/norm_d  ) %u=v_k -x_t
           ...
           for j=0:n^(dim)-1;
               ...
               points_ar=[];
                ...
                value=d_function(points_ar,p_d(length(coeff_d),:),epsilon);
                d_k_array(j+1)=d_k_array(j+1)+ coeff_d(length(coeff_d))*value;
           end
       else 
           valflag=2;
           lambda=value2;
           ...
           d_k_array_copy=d_k_array;
           d_k_array= (1- lambda/ norm_d )*d_k_array;
       end
       ...
       if (valflag==1)
               d_k_x_t=d_k_x_t + lambda*inner_product(c,[1],p,p_d_k,epsilon,dim,functype);%<足された点,x_t>
               d_k_mu=d_k_mu+lambda*mu(ind_v);
               d_k_norm=d_k_norm + lambda^2*basevalue +2*lambda*inner_product(coeff_d_copy,[1],p_d_copy,p_d_k,epsilon,dim,functype) ; %足された点 |足された点|_k ^2
       else               
               d_k_x_t= ( 1-  lambda/norm_d )*d_k_x_t ;
               d_k_mu= ( 1-  lambda/norm_d )*d_k_mu;
               d_k_norm=( 1-  lambda/norm_d )^2*d_k_norm;
       end
